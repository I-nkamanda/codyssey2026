...
## 왜 컨테이너 내부 포트로 직접 접속할 수 없는가?

Flask라든가 FastAPI, 심지어 npm으로 실행하는 웹서버 등은 기본적으로 localhost:8080 이나 localhost:3000 등에서 실행된다. 코드 안에 특정 포트로 실행합니다! 하고 명시해두면, 그 포트로 접속하면 해당 앱에 접속할 수 있다.

하지만 컨테이너는 기본적으로 외부와 격리되어 동작한다. 이는 보안상 이점이 있지만, 외부로부터 직접 컨테이너로의 접속은 불가능하게 만든다. 컨테이너는 호스트와 분리된 네트워크 네임스페이스 안에서 동작하기 때문이다. 
즉, 컨테이너의 `80`번 포트는 호스트의 `80`번 포트와 같은 공간이 아니다.

그래서 호스트 브라우저에서 바로 컨테이너 내부 IP/포트에 접근하지 않는 한,
일반적으로는 `-p 8080:80` 같은 포트 매핑이 필요하다.

포트 매핑이 필요한 이유는 다음과 같다.
- 호스트의 요청을 컨테이너 내부 서비스로 전달하기 위해
- 여러 컨테이너의 같은 내부 포트(예: 모두 80번)를 서로 다른 호스트 포트로 구분하기 위해
- 외부 노출 여부를 명시적으로 통제하기 위해
